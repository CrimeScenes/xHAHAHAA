...
local TargetESP = nil  -- ESP variable
...
local function DrawESP(targetPlayer)
    ...
    if humanoid and humanoid.Health > 0 then
        -- Create the ESP box or outline here (using the 'Drawing' library)
        -- Example: Draw a box around the target
        local targetPos = character:FindFirstChild("Head").Position
        local screenPos, onScreen = workspace.CurrentCamera:WorldToScreenPoint(targetPos)
        
        if onScreen then
            -- Example box drawing (you can customize the visuals as needed)
            local box = Drawing.new("Square")
            box.Size = Vector2.new(100, 100)  -- Adjust size
            box.Position = Vector2.new(screenPos.X - 50, screenPos.Y - 50)  -- Adjust position
            box.Color = Color3.fromRGB(255, 0, 0)  -- Red color for ESP
            box.Thickness = 2
            box.Visible = true
